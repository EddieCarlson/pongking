...
def add_player(request):
    if not League.objects.all():
        l = League()
        l.name = "apartment"
        l.save()
    else:
        l = League.objects.get(pk=1)
    message = ""
...
def mobile_rank(request):
    players = sorted(Player.objects.all(), key=lambda x: x.level, reverse=True)#TODO Eddie make this eventually for leagues
